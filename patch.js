...
function createTileUnderHUD() {
  ...
  return true;
    ...
    // === 1. Глобальное состояние навигатора ===
    content = replaceOnce(
      content,
      `// === Отладка ===
...
let navTarget = null; // { x, y, label } — цель в тайлах`,
    ...
    // === 2. UI навигатора — добавим в createHUDs ===
    content = replaceOnce(
      content,
      `function createHUDs() {`,
  ...
  panel.innerHTML = \`
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px; margin-bottom: 8px;">
      <button id="navFindGrass" style="padding: 6px; background: #2a6a2a; 
        border: none; border-radius: 4px; color: white; cursor: pointer; font-size: 11px;">
        🌿 Траву
      </button>
      <button id="navFindTree" style="padding: 6px; background: #3a5a2a; 
        border: none; border-radius: 4px; color: white; cursor: pointer; font-size: 11px;">
        🌲 Дерево
      </button>
      <button id="navFindBot" style="padding: 6px; background: #6a2a2a; 
        border: none; border-radius: 4px; color: white; cursor: pointer; font-size: 11px;">
        👹 Бота
      </button>
      <button id="navReset" style="padding: 6px; background: #444; 
        border: none; border-radius: 4px; color: white; cursor: pointer; font-size: 11px;">
        ❌ Сброс
      </button>
    </div>

    <div id="navStatus" style="font-size: 11px; color: #888; 
      padding-top: 6px; border-top: 1px solid #333; min-height: 16px;">
      Готов к поиску
    </div>
  \`;
  ...
  document.getElementById('navCellInput').addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      ...
      if (isNaN(id) || id < 0 || id >= MAP_SIZE * MAP_SIZE) {
        setNavStatus('❌ Неверный ID', '#ff6666');
        return;
      }
...
function resetNav() {
  ...
  document.getElementById('navCellInput').value = '';
}

function goToTile(tx, ty, label) {
  if (currentScene !== 'world') {
    setNavStatus('❌ Сначала выйди из города', '#ff6666');
    return;
  }

  if (!isWalkable(tx, ty)) {
    setNavStatus('❌ Клетка непроходима', '#ff6666');
    return;
  }
  ...
  if (!path) {
    setNavStatus('❌ Путь не найден', '#ff6666');
    return;
  }

  if (path.length === 0) {
    setNavStatus('📍 Уже там', '#88ff88');
    return;
  }
...
function findNearest(type) {
  ...
  for (let ty = 0; ty < MAP_SIZE; ty++) {
    for (let tx = 0; tx < MAP_SIZE; tx++) {
      ...
      let match = false;
      if (type === 'grass') {
        match = (tile === TILE.GRASS || tile === TILE.GRASS_DARK || tile === TILE.GRASS_LIGHT);
      } else if (type === 'tree') {
        match = (tile === TILE.TREE);
      }
      if (!match) continue;
  ...
  if (!best) {
    setNavStatus('❌ Ничего не найдено', '#ff6666');
    return;
  }
...
function createHUDs() {`,
  ...
  const me = players.get(myId) || character;`,
    ...
    // === 4. Вызов createNavigatorPanel при старте игры ===
    content = replaceOnce(
      content,
      `  createHUDs();
  connectSocket();`,
